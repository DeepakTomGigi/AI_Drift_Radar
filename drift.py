...
#     """
#     Population Stability Index scaled to 0-100.
#     expected & actual should be 1D arrays for a single feature (or concatenated multi-features).
#     """
#     expected = np.array(expected).ravel()
#     actual = np.array(actual).ravel()
#     # use KBinsDiscretizer to create buckets
#     kbd = KBinsDiscretizer(n_bins=buckets, encode='ordinal', strategy='quantile')
#     try:
#         all_vals = np.concatenate([expected, actual]).reshape(-1, 1)
#         kbd.fit(all_vals)
#         e_bins = kbd.transform(expected.reshape(-1,1)).ravel()
#         a_bins = kbd.transform(actual.reshape(-1,1)).ravel()
#     except Exception:
#         return 0.0
#
#     psi = 0.0
...
#     return float(min(max(psi * 100, 0.0), 100.0))
#
...
#         self.buffer.append((pred, true))
#
#     def score(self):
#         if len(self.buffer) < 5:
#             return 0.0
#         acc = sum(1 for p,t in self.buffer if p==t) / len(self.buffer)
#         # drift = drop in accuracy relative to perfect accuracy
#         return (1.0 - acc) * 100.0
#
# def anomaly_score(sample, baseline_mean, baseline_std):
#     """
#     sample: 1D array of features
#     baseline_mean/std: dict or 1D array
#     returns a 0-100 anomaly percentage (mean of absolute z-scores scaled)
#     """
#     s = np.array(sample, dtype=float)
#     m = np.array([baseline_mean[k] for k in baseline_mean])
#     sd = np.array([baseline_std[k] for k in baseline_std])
#     sd = np.where(sd == 0, 1e-6, sd)
#     z = np.abs((s - m) / sd)
#     score = np.mean(z) * 10  # scale factor
#     return float(min(max(score, 0.0), 100.0))
#
# def embedding_drift_from_texts(baseline_emb_mean, texts):
#     """
#     texts: list of string representations of recent rows
#     compute mean embedding and cosine distance to baseline centroid
#     """
#     embs = emb_model.encode(texts, show_progress_bar=False)
#     mean_emb = np.mean(embs, axis=0)
#     sim = 1 - cosine(baseline_emb_mean, mean_emb)
#     # sim in [-1,1]; convert to 0-100 drift (1 -> 0 drift; lower sim -> higher drift)
#     drift = (1 - sim) * 100
...
Contains all drift calculation functions
"""
